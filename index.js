...
const uri = `mongodb+srv://admin:${process.env.DB_PASS}@cluster0.jq0hlcl.mongodb.net/?retryWrites=true&w=majority`;
...
async function run() {
    try {
        ...
        app.get("/tools/home", async (req, res) => {
            const query = {};
            const tools = await drillCollection
                .find(query)
                .limit(6)
                .sort({ _id: -1 })
                .toArray();
        ...
        app.put("/drill/:id", async (req, res) => {
            ...
            const result = drillCollection.updateOne(
                filter,
                updatedDoc,
                options
            );
        ...
        app.patch("/payment/:id", verifyJWT, async (req, res) => {
            ...
            const updatedPurchases = await purchaseCollection.updateOne(
                filter,
                updatedDoc
            );
